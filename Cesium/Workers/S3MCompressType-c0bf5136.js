 ...
 */
define(['exports', './when-8d13db60', './Check-70bec281', './Cartographic-f27b0939', './Cartesian2-09435a6c', './BoundingSphere-c409f092', './Cartesian4-5af5bb24', './RuntimeError-ba10bc3e', './WebGLConstants-4c11ee5f', './ComponentDatatype-5862616f', './PrimitiveType-97893bc7', './IndexDatatype-9435b55f', './BoundingRectangle-3d4f3d01'], function (exports, when, Check, Cartographic, Cartesian2, BoundingSphere, Cartesian4, RuntimeError, WebGLConstants, ComponentDatatype, PrimitiveType, IndexDatatype, BoundingRectangle) {
    ...
    S3MEdgeProcessor.RegularInstanceStride = 11;//每个实例的步进长度
    ...
    function createEdgeSidenessVertexBuffer(context){
        if(when.defined(context.cachedSidenessVertexBuffer)){
            return context.cachedSidenessVertexBuffer;
        }
        ...
        context.cachedSidenessVertexBuffer = Buffer.createVertexBuffer({
            context : context,
            typedArray : typedArray,
            usage : BufferUsage.STATIC_DRAW
        });
        context.cachedSidenessVertexBuffer.vertexArrayDestroyable = false;
        return context.cachedSidenessVertexBuffer;
    }
    ...
    S3MEdgeProcessor.createIndexBuffer = function(context){
        if(when.defined(context.cachedSidenessIndexBuffer)){
            return context.cachedSidenessIndexBuffer;
        }

        context.cachedSidenessIndexBuffer = Buffer.createIndexBuffer({
            context : context,
            typedArray : createEdgeIndexArray(),
            usage : BufferUsage.STATIC_DRAW,
            indexDatatype : IndexDatatype.IndexDatatype.UNSIGNED_SHORT
        });
        context.cachedSidenessIndexBuffer.vertexArrayDestroyable = false;
        return context.cachedSidenessIndexBuffer;
    };
    ...
    S3MEdgeProcessor.createRegularEdgeAttributes = function(context, regular){
        ...
        var stride = S3MEdgeProcessor.RegularInstanceStride;//3 + 3 + 3 + 1 + 1  pos0 pos1 facenormal0 variantStroke variantExtension
        ...
        attributeLocations['aVariantStroke'] = attIndex++;
        attributes.push({
            index : attributeLocations['aVariantStroke'],
            vertexBuffer : instanceBuffer,
        ...
        attributeLocations['aVariantExtension'] = attIndex++;
        attributes.push({
            index : attributeLocations['aVariantExtension'],
            vertexBuffer : instanceBuffer,
            componentsPerAttribute : 1,
            componentDatatype : ComponentDatatype.ComponentDatatype.FLOAT,
            normalize              : true,
    ...
    S3MEdgeProcessor.createSilhouetteEdgeAttributes = function(context, silhouette){
        ...
        // pos0 pos1 facenormal0 facenormal1 variantStroke variantExtension
        var stride = 3 + 3 + 3 + 3 + 1 + 1;
    ...
    S3MEdgeProcessor.createEdgeDataByIndices = function(vertexPackage, indexPackage){
        ...
        for(var i = 0,j = Math.floor(len / 4) * 4; i < j; i += 4){
            ...
            if(i3 === i4){
                ...
                var variants = getVariantParams(scratchV0$1, scratchV1$1);
                var variantStroke = variants.variantStroke;
                var variantExtension = variants.variantExtension;
                regularBuffer.push(variantStroke);
                regularBuffer.push(variantExtension);
            }
            else{
                Cartographic.Cartesian3.subtract(scratchV1$1, scratchV0$1, scratchN0);
                Cartographic.Cartesian3.subtract(scratchV2$1, scratchV0$1, scratchN1);
                ...
                var variantStroke = variants.variantStroke;
                var variantExtension = variants.variantExtension;
                silhouetteBuffer.push(variantStroke);
                silhouetteBuffer.push(variantExtension);
            }
    ...
    function getVariantParams(scratchV0, scratchV1) {
        function edgeHashFunction(v0, v1) {
            var D = new Float32Array(6);
            var H = new Uint32Array(D.buffer);
        ...
        function getIntRange(seed, min, max) {
            max -= min;
            var newSeed = (48217 * seed) % 2147483647;
            var b = newSeed / 2147483647;
            var result =  min + b * max;
            return {
                seed: newSeed,
        ...
        function getFloat(seed) {
            var newSeed = (48217 * seed) % 2147483647;
            var result =  newSeed / (2147483647 - 1);
            return {
                seed: newSeed,
                result: result
            }
        ...
        var seed = edgeHashFunction(scratchV0, scratchV1);
        if(!when.defined(seed)) {
            seed = Math.random() * 2147483647;
        }
        var obj = getIntRange(seed, 0, 255);
        seed = obj.seed;
        var N = obj.result;
        obj = getIntRange(seed, 0, 5);
        seed = obj.seed;
        var O = obj.result;
        obj = getFloat(seed);
        seed = obj.seed;
        var P = obj.result;
        P = -(1 - Math.min(P / 0.7, 1)) + Math.max(0, P - 0.7) / (1 - 0.7);
        P = 255 * (Math.abs(P) ** 1.2 * (0 > P ? -1 : 1) * 0.5 + 0.5);
        var variantStroke = O;
        var variantExtension = P;
        return {
            variantStroke: variantStroke,
